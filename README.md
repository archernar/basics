...
| command_exists(command) | pidof_name(name) |
| contains(string,substring) | prompter(question,default:-}") |
| cp_r(filename1,filename2) | remove_leading_whitespace(input) |
| create_markdown_table2(input_file,str) | repeat_char(char,count) |
| create_markdown_table3(input_file,str) | replace(string,old,new) |
| create_markdown_table4(input_file,str) | rm_rf(filename) |
| current_timestamp(void) | rm_rf_silent(filename) |
| dirname(path) | run_and_error(command) |
